...
def make_graph(hits):
    ...
    if len(hits) < 8:  # Need at least 4 points for k=4
...
def train_diffusion_model_batched(embeddings, n_steps=1000, epochs=200, batch_size=8):
    ...
    # ADDED: List to store loss history
    loss_history = []
    ...
    # MODIFIED: Return model and loss history
...
def train_autoencoder(gnn, decoder, graph_data, target_particles, epochs=300, batch_size=8):  # Increased epochs
    ...
    optimizer = optim.AdamW(list(gnn.parameters()) + list(decoder.parameters()), lr=5e-4, weight_decay=1e-5)
    ...
    # MODIFIED: Return GNN, decoder, and loss history
    return gnn, decoder, loss_history
...
def main_pipeline_jetnet(batch_size=8):
    ...
    graph_data, target_particles, original_jets = collect_graph_and_targets(particle_data, jet_data, max_jets=200000)
    ...
    # MODIFIED: Get loss history from training function
    gnn, decoder, ae_loss_history = train_autoencoder(
        gnn, decoder, graph_data, target_particles, epochs=300, batch_size=batch_size
    )

    # ADDED: Plot autoencoder loss curve
    print("\n=== Plotting Autoencoder Loss Curve ===")
    plt.figure(figsize=(10, 6))
    plt.plot(ae_loss_history)
    plt.xlabel('Epoch')
    plt.ylabel('Average Loss')
    plt.title('Autoencoder (GNN + Decoder) Training Loss')
    ...
    # MODIFIED: Get loss history from training function
    diff_model, diff_loss_history = train_diffusion_model_batched(
        all_pooled_embeddings, epochs=300, batch_size=batch_size
    )

    # ADDED: Plot diffusion model loss curve
    print(f"\n=== Plotting Diffusion Model Loss Curve ===")
    plt.figure(figsize=(10, 6))
    plt.plot(diff_loss_history)
    plt.xlabel('Epoch')
    ...
    return {
        ...
        'jet_data': jet_data,
        # ADDED: Store histories in results dict
